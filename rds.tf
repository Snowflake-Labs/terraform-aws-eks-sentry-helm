...
module "sentry_rds_pg" {
  ...
  name           = "${replace(var.module_prefix, "_", "-")}-pg11-db"
...
resource "aws_db_parameter_group" "aurora_db_postgres11_parameter_group" {
  name        = "aurora-${replace(var.module_prefix, "_", "-")}-db-postgres11-parameter-group"
  family      = "aurora-postgresql11"
  description = "aurora-db-postgres11-parameter-group"
}

resource "aws_rds_cluster_parameter_group" "aurora_cluster_postgres11_parameter_group" {
  name        = "aurora-${replace(var.module_prefix, "_", "-")}-postgres11-cluster-parameter-group"
  family      = "aurora-postgresql11"
